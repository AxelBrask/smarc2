...
class SamJointStateConverter(Node):
    ...
    def timer_callback(self, event):
        ...
        duration = (self.get_clock().now() - self.start_time).to_sec()
    ...
    def __init__(self):
        ...
        super().__init__("sam_joint_state_converter", anonymous=True)
        self.start_time = self.get_clock().now()
        self.velocities = [0., 0.]

        self.thruster_sub = self.create_subscription(ThrusterRPMs,Topics.RPM_CMD_TOPIC, self.thruster_callback)
        self
        # self.vector_sub = self.create_subscription(ThrusterAngles,"~thrust_vector_cmd",  self.vector_callback)
        self.vector_sub = self.create_subscription(ThrusterAngles,Topics.THRUST_VECTOR_CMD_TOPIC,  self.vector_callback)

        self.joint_state_pub = self.create_publisher(JointState,"command_states",  queue_size=10)
...
def main(args=None):
    ...
    try:
        rclpy.spin(sam_joint_state_converter)
    except:
