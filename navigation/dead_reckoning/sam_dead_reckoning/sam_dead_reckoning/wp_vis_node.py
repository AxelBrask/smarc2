...
class PublishWPsVis(Node):
    ...
    def __init__(self, namespace=None):
        ...
        self.client = self.create_client(UTMLatLon, Topics.UTM_LATLON_CONVERSION_SERVICE,callback_group= self.callback_group)
        if not self.client.wait_for_service(timeout_sec=5.0):
            self.get_logger().error('No UTM to lat/lon conversion service available')

        # GPS odom in UTM frame
        self.gps_sam_sub = self.create_subscription(GotoWaypoint,self.wp_topic, self.wp_vis_cb, 10, callback_group=self.callback_group)
...
def main(args=None, namespace=None):
    ...
    try:
        ...
        rclpy.spin(dr_visual, executor=executor)
